...
%if %{with quadmath}
BuildRequires: devtoolset-8-libquadmath-devel
%endif
BuildRequires: bison
...
cat >> ./tools/build/src/user-config.jam << EOF
using python : 3.5 : /opt/_internal/cpython-3.5.9/bin/python3 : /opt/_internal/cpython-3.5.9/include/python3.5m : /opt/_internal/cpython-3.5.9/lib ;
using python : 3.6 : /opt/_internal/cpython-3.6.10/bin/python3 : /opt/_internal/cpython-3.6.10/include/python3.6m : /opt/_internal/cpython-3.6.10/lib ;
using python : 3.7 : /opt/_internal/cpython-3.7.7/bin/python3 : /opt/_internal/cpython-3.7.7/include/python3.7m : /opt/_internal/cpython-3.7.7/lib ;
using python : 3.8 : /opt/_internal/cpython-3.8.3/bin/python3 : /opt/_internal/cpython-3.8.3/include/python3.8 : /opt/_internal/cpython-3.8.3/lib ;
EOF
# %if %{with python3}
# cat >> ./tools/build/src/user-config.jam << EOF
...
echo ============================= build serial ==================
CXXFLAGS=-fPIC CFLAGS=-fPIC ./b2 -d+2 -q %{?_smp_mflags} \
...
CXXFLAGS=-fPIC CFLAGS=-fPIC ./b2 -q %{?_smp_mflags} \
	--with-mpi --with-graph_parallel --build-dir=$MPI_COMPILER \
	--stagedir=${RPM_BUILD_ROOT}${MPI_HOME} \
	variant=release threading=multi debug-symbols=on pch=off \
	python=3.5,3.6,3.7,3.8 stage

%global python35_sitearch /opt/python/cp35-cp35m/lib/python3.5/site-packages
%global python36_sitearch /opt/python/cp36-cp36m/lib/python3.6/site-packages
%global python37_sitearch /opt/python/cp37-cp37m/lib/python3.7/site-packages
%global python38_sitearch /opt/python/cp38-cp38/lib/python3.8/site-packages
%global python39_sitearch /opt/python/cp39-cp39/lib/python3.9/site-packages

# Move Python module to proper location for automatic loading
mkdir -p ${RPM_BUILD_ROOT}%{python35_sitearch}/openmpi/boost
touch ${RPM_BUILD_ROOT}%{python35_sitearch}/openmpi/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.5/mpi.so \
   ${RPM_BUILD_ROOT}%{python35_sitearch}/openmpi/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python36_sitearch}/openmpi/boost
touch ${RPM_BUILD_ROOT}%{python36_sitearch}/openmpi/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.6/mpi.so \
   ${RPM_BUILD_ROOT}%{python36_sitearch}/openmpi/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python37_sitearch}/openmpi/boost
touch ${RPM_BUILD_ROOT}%{python37_sitearch}/openmpi/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.7/mpi.so \
   ${RPM_BUILD_ROOT}%{python37_sitearch}/openmpi/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python38_sitearch}/openmpi/boost
touch ${RPM_BUILD_ROOT}%{python38_sitearch}/openmpi/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.8/mpi.so \
   ${RPM_BUILD_ROOT}%{python38_sitearch}/openmpi/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python39_sitearch}/openmpi/boost
touch ${RPM_BUILD_ROOT}%{python39_sitearch}/openmpi/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.9/mpi.so \
   ${RPM_BUILD_ROOT}%{python39_sitearch}/openmpi/boost/
...
# Removenumpy cmake files (some of these are duplicates of the generic bits anyway).
...
# Move Python module to proper location for automatic loading
mkdir -p ${RPM_BUILD_ROOT}%{python35_sitearch}/mpich/boost
touch ${RPM_BUILD_ROOT}%{python35_sitearch}/mpich/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.5/mpi.so \
   ${RPM_BUILD_ROOT}%{python35_sitearch}/mpich/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python36_sitearch}/mpich/boost
touch ${RPM_BUILD_ROOT}%{python36_sitearch}/mpich/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.6/mpi.so \
   ${RPM_BUILD_ROOT}%{python36_sitearch}/mpich/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python37_sitearch}/mpich/boost
touch ${RPM_BUILD_ROOT}%{python37_sitearch}/mpich/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.7/mpi.so \
   ${RPM_BUILD_ROOT}%{python37_sitearch}/mpich/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python38_sitearch}/mpich/boost
touch ${RPM_BUILD_ROOT}%{python38_sitearch}/mpich/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.8/mpi.so \
   ${RPM_BUILD_ROOT}%{python38_sitearch}/mpich/boost/

mkdir -p ${RPM_BUILD_ROOT}%{python39_sitearch}/mpich/boost
touch ${RPM_BUILD_ROOT}%{python39_sitearch}/mpich/boost/__init__.py
mv ${RPM_BUILD_ROOT}${MPI_HOME}/lib/boost-python3.9/mpi.so \
   ${RPM_BUILD_ROOT}%{python39_sitearch}/mpich/boost/
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/libboost_numpy35.so.%{sonamever}
%{_libdir}/libboost_numpy36.so.%{sonamever}
%{_libdir}/libboost_numpy37.so.%{sonamever}
%{_libdir}/libboost_numpy38.so.%{sonamever}
%{_libdir}/libboost_numpy39.so.%{sonamever}
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/libboost_python35.so.%{sonamever}
%{_libdir}/libboost_python36.so.%{sonamever}
%{_libdir}/libboost_python37.so.%{sonamever}
%{_libdir}/libboost_python38.so.%{sonamever}
%{_libdir}/libboost_python39.so.%{sonamever}
...
%{_libdir}/libboost_numpy35.so
%{_libdir}/libboost_numpy36.so
%{_libdir}/libboost_numpy37.so
%{_libdir}/libboost_numpy38.so
%{_libdir}/libboost_numpy39.so
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/openmpi/lib/libboost_mpi_python35.so.%{sonamever}
%{_libdir}/openmpi/lib/libboost_mpi_python36.so.%{sonamever}
%{_libdir}/openmpi/lib/libboost_mpi_python37.so.%{sonamever}
%{_libdir}/openmpi/lib/libboost_mpi_python38.so.%{sonamever}
%{python35_sitearch}/openmpi/boost/
%{python36_sitearch}/openmpi/boost/
%{python37_sitearch}/openmpi/boost/
%{python38_sitearch}/openmpi/boost/
%{python39_sitearch}/openmpi/boost/

%files openmpi-python3-devel
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/openmpi/lib/libboost_mpi_python35.so
%{_libdir}/openmpi/lib/libboost_mpi_python36.so
%{_libdir}/openmpi/lib/libboost_mpi_python37.so
%{_libdir}/openmpi/lib/libboost_mpi_python38.so
%{_libdir}/openmpi/lib/libboost_mpi_python39.so
...
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/mpich/lib/libboost_mpi_python35.so.%{sonamever}
%{_libdir}/mpich/lib/libboost_mpi_python36.so.%{sonamever}
%{_libdir}/mpich/lib/libboost_mpi_python37.so.%{sonamever}
%{_libdir}/mpich/lib/libboost_mpi_python38.so.%{sonamever}
%{_libdir}/mpich/lib/libboost_mpi_python39.so.%{sonamever}
%{python35_sitearch}/mpich/boost/
%{python36_sitearch}/mpich/boost/
%{python37_sitearch}/mpich/boost/
%{python38_sitearch}/mpich/boost/
%{python39_sitearch}/mpich/boost/

%files mpich-python3-devel
%defattr(-, root, root, -)
%doc LICENSE_1_0.txt
%{_libdir}/mpich/lib/libboost_mpi_python35.so
%{_libdir}/mpich/lib/libboost_mpi_python36.so
%{_libdir}/mpich/lib/libboost_mpi_python37.so
%{_libdir}/mpich/lib/libboost_mpi_python38.so
%{_libdir}/mpich/lib/libboost_mpi_python39.so
