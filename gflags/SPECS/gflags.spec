...
%build
cmake -DBUILD_TESTING:BOOL=ON -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_SHARED_LIBS=ON -DLIB_INSTALL_DIR=%{_libdir} .
make %{?_smp_mflags}

%install
make install DESTDIR=%{buildroot} INSTALL="install -p"
rm -rf %{buildroot}/root/.cmake

%check
LD_LIBRARY_PATH=%{buildroot}/usr/lib ctest --output-on-failure
...
%files
%doc ChangeLog.txt README.md COPYING.txt AUTHORS.txt
